...
public class GameManager : MonoBehaviour
{
	...
	public UnityEvent OnRewardedIntComplete;
	...
	[SerializeField] MassegeSystem massegeSystem;
	...
	[SerializeField] string[] musicLoop;	
	...
	{
		get { return PlayerPrefs.GetInt("MusicIndex", 0); }
		set { PlayerPrefs.SetInt("MusicIndex", value); }
	}

	int CurrentMusicIndex()
    {
		MusicIndex++;
		if(MusicIndex< musicLoop.Length)
        {
			return MusicIndex;
	...
	void Start()
	{
		...
		OnEnterStateMenus();
		massegeSystem.closeButton.onClick.AddListener(CloseMassege);
		StartCoroutine( AdmobAdmanager.Instance.ShowBanner());
    ...
    {
		massegeSystem.massegePnel.SetActive(true);
		massegeSystem.header.text = header;
		massegeSystem.massege.text = massege;
		
    }
	void CloseMassege()
    {
		massegeSystem.massegePnel.SetActive(false); 
	}
	...
	private void OnEnterStateMenus()
	{
		...
		MusicManager.PlayMusic("IntroLoop1");
	...
	private void OnEnterStateGame()
	{
		...
		MusicManager.PlayMusic(musicLoop[CurrentMusicIndex()]);
    ...
    public void  GiveRward()
    {
		TotalCredits += 100;
		ShowMassege("Congratution", "You Got 100 Credits");		
	}
